...
namespace MoreMountains.Tools
{
        ...
        MMEventListener<MMSoundManagerTrackFadeEvent>
    {
        ...
        protected virtual void InitializeSoundManager()
        {
            ...
            _pool.FillAudioSourcePool(AudioSourcePoolSize, transform);
        ...
        public virtual AudioSource PlaySound(AudioClip audioClip, MMSoundManagerPlayOptions options)
        {
            return PlaySound(audioClip, options.MmSoundManagerTrack, options.Location,
                ...
                options.DoNotAutoRecycleIfNotDonePlaying, options.PlaybackTime, options.PlaybackDuration,
                ...
                options.UseSpatialBlendCurve, options.SpatialBlendCurve, options.UseReverbZoneMixCurve,
                options.ReverbZoneMixCurve,
        ...
        /// <returns></returns>
        public virtual AudioSource PlaySound(AudioClip audioClip, MMSoundManagerTracks mmSoundManagerTrack,
            Vector3 location,
        ...
        )
        {
            ...
            if (!audioSource)
            {
                ...
                audioSource = _pool.GetAvailableAudioSource(PoolCanExpand, transform);
            ...
            {
                audioSource = audioResourceToPlay;
            }
        ...
        protected virtual void ControlTrack(MMSoundManagerTracks track, ControlTrackModes trackMode,
            float volume = 0.5f)
        {
            string target = "";
        ...
        public virtual void FadeTrack(MMSoundManagerTracks track, float duration, float initialVolume = 0f,
            float finalVolume = 1f, MMTweenType tweenType = null)
        {
            Coroutine coroutine =
        ...
        public virtual void FadeSound(AudioSource source, float duration, float initialVolume, float finalVolume,
            MMTweenType tweenType, bool freeAfterFade = false)
        {
        ...
        /// <param name="tweenType"></param>
        /// <returns></returns>
        protected virtual IEnumerator FadeTrackCoroutine(MMSoundManagerTracks track, float duration,
            float initialVolume, float finalVolume, MMTweenType tweenType)
        {
        ...
        protected virtual IEnumerator FadeCoroutine(AudioSource source, float duration, float initialVolume,
            float finalVolume, MMTweenType tweenType, bool freeAfterFade = false)
        {
            float startedAt = Time.unscaledTime;
            if (tweenType == null)
            {
        ...
        public virtual void OnMMEvent(MMSoundManagerTrackFadeEvent trackFadeEvent)
        {
            switch (trackFadeEvent.Mode)
            {
                case MMSoundManagerTrackFadeEvent.Modes.PlayFade:
                    FadeTrack(trackFadeEvent.Track, trackFadeEvent.FadeDuration,
                        settingsSo.GetTrackVolume(trackFadeEvent.Track), trackFadeEvent.FinalVolume,
                        trackFadeEvent.FadeTween);
                    break;
        ...
        public virtual void OnMMEvent(MMSoundManagerSoundFadeEvent soundFadeEvent)
        {
            ...
            switch (soundFadeEvent.Mode)
            {
                case MMSoundManagerSoundFadeEvent.Modes.PlayFade:
                    if (_tempAudioSource != null)
                    {
                        FadeSound(_tempAudioSource, soundFadeEvent.FadeDuration, _tempAudioSource.volume,
                            soundFadeEvent.FinalVolume,
