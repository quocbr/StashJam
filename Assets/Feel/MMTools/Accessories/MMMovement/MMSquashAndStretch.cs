...
namespace MoreMountains.Tools
{
    ...
    public class MMSquashAndStretch : MonoBehaviour
    {
        ...
        [MMInformation(
            "This component will apply squash and stretch based on velocity (either position based or computed from a Rigidbody. It has to be put on an intermediary level in the hierarchy, between the logic (top level) and the model (bottom level).",
        ...
        [Tooltip("the maximum duration of the squash (will be reduced if velocity is low)")] [MMVector("Min", "Max")]
        public Vector2 SquashDuration = new Vector2(0.25f, 0.5f);

        /// the maximum intensity of the squash
        [Tooltip("the maximum intensity of the squash")] [MMVector("Min", "Max")]
        ...
        [Tooltip("the damping to apply to the spring")] [MMCondition("Spring", true)]
        public float SpringDamping = 0.3f;

        /// the spring's frequency
        [Tooltip("the spring's frequency")] [MMCondition("Spring", true)]
        ...
        public virtual float TimescaleTime => Timescale == Timescales.Regular ? Time.time : Time.unscaledTime;

        public virtual float TimescaleDeltaTime =>
            Timescale == Timescales.Regular ? Time.deltaTime : Time.unscaledDeltaTime;
        ...
        protected virtual void ComputeVelocityAndDirection()
        {
            ...
            if (AutoSquashOnStop)
            {
                ...
                else if (_movementStarted)
                {
                    ...
                    _squashing = true;
                    float duration = MMMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashDuration.x,
                        SquashDuration.y);
                    float intensity = MMMaths.Remap(_lastVelocity, 0f, MaximumVelocity, SquashIntensity.x,
                        SquashIntensity.y);
        ...
        protected virtual void ComputeNewLocalScale()
        {
            ...
            if (Spring)
            {
                MMMaths.Spring(ref _springScale, _newLocalScale, ref _springVelocity, SpringDamping, SpringFrequency,
                    Time.deltaTime);
