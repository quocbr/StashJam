...
namespace HellTap.PoolKit {
	...
	public class DespawnerEventOnPhysicsOverlap : DespawnerEvent {
		...
		// Setup contact filter helper for new physics methods
		...
		void FixedUpdate () {
			...
			// Overlap Circle 2D
				...
				// Check Collisions
				#if UNITY_2023_1_OR_NEWER	// Physics2D.OverlapBoxNonAlloc is now marked as deprecated

					// Create and configure ContactFilter2D
					_contactFilter.SetLayerMask(despawner.filterLayers);

					// Use the new Physics2D.OverlapCircle method
					m_HitDetect = Physics2D.OverlapCircle(
					    m_Transform.position + m_Transform.TransformDirection( despawner.overlapOffset ),
					    liveScale.x,
					    _contactFilter,
					    _resultsBuffer2D
					);
			...
			// Overlap Box 2D
				...
				// Check Collisions
					...
					// Do the overlap box
					m_HitDetect = Physics2D.OverlapBox(m_Transform.position + m_Transform.TransformDirection(despawner.overlapOffset), liveScale, m_Transform.eulerAngles.z, _contactFilter, _resultsBuffer2D);
